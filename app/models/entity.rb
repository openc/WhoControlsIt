...
class Entity < ActiveRecord::Base
  ...
  validates :jurisdiction, :allow_nil => true, :inclusion => { in: Rails.application.config.countries.collect{ |x| x[1]}}
  ...
  def jurisdiction_name()
    return Hash[Rails.application.config.countries].invert[jurisdiction]
  end
end
